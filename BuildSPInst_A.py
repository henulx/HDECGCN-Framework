...
class GetInst_A(object):
    def __init__(self,useful_sp_lab, img3d, gt, trops):
        ...
        trops = np.array(trops, dtype='int')
        self.trops = (trops[:,0]-1)*self.c+trops[:,1]-1
        ...
        self.sp_label_vec = []
        self.sp_A = []
        Q = np.zeros([self.r*self.c,self.sp_num],dtype=np.float32)
        self.Q = Q
        self.support = []
        self.CalSpMean()
        self.CalSpNei()
    ...
    def CalSpMean(self):
        self.gt_tr[self.trops] = self.gt1d[self.trops]
        mark_mat = np.zeros([self.r*self.c])
        mark_mat[self.trops] = -1
