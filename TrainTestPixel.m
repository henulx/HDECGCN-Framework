function [ intr, inte, labeltrain, labeltest, trpos2, tepos2 ] = TrainTestPixel( img, gt, num_tr, cand )
%%
c = 1:max(max(gt));
intr = [];inte = [];labeltrain = [];labeltest = [];trpos = [];tepos = [];
[r1,c1,s1] = size(img);
img = reshape(img,r1*c1,s1);
gt = reshape(gt,r1*c1,1);%21025*1
for i = 1:size(c,2 )%1-16
    pos = find(gt == i);
    index1 = randperm(length(pos));
    if(size(pos,1) <= num_tr)
        trpos1 = pos(index1(1:cand));
        tepos1 = pos(index1(cand+1:length(index1)));
    else
        trpos1 = pos(index1(1:num_tr(:,i)));
        tepos1 = pos(index1(num_tr(:,i)+1:length(index1)));
    end
